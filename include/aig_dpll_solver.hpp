...
class aig_dpll_solver {
    ...
    public:
        ...
        bool BCP(GateId id){
            ...
            for (const auto& out : ii_table[val][id]){
                if (out<m_original_gate_size){
                    ...
                    if (lwv_it == learn_watch_vals.end()) continue;
                    ...
                    if (count_watch == k) {
                        ...
                        std::unordered_set<GateId> in_conf;
                        for (GateId f : fanins) {
                            if (in_conf.insert(f).second) conf_lines.push_back(f);
                        }
                        return false;
                    }
                    ...
                    if (count_unassigned == 1 && count_watch == k - 1 && last_unassigned != INVALID_GATE) {
                        ...
                        size_t idx = SIZE_MAX;
                        for (size_t i = 0; i < k; ++i) {
                            if (fanins[i] == last_unassigned) { idx = i; break; }
                        }
                        if (idx == SIZE_MAX) continue; // shouldn't happen
                        ...
                        if (!BCP(last_unassigned)) return false;
                    }
        ...
        void backtrack(int target_level){
            if (target_level >= cur_level) return;
        ...
        bool conflict(){
            ...
            if (conf_lines.empty()) return false;
            ...
            do{
                iteration++;
                conf_lines.erase(
                    std::remove_if(conf_lines.begin(), conf_lines.end(),
                ...
                conf_lines.erase(
                    std::remove_if(conf_lines.begin(), conf_lines.end(),
                    [&](GateId x){ return std::find(expand_list.begin(), expand_list.end(), x) != expand_list.end(); }
                    ),
                conf_lines.end()    
                );
            ...
            std::sort(conf_lines.begin(), conf_lines.end(),
                    [&](GateId a, GateId b){
                        return decision_level[a] > decision_level[b];
                    });
            GateId uip  = conf_lines[0]; 
