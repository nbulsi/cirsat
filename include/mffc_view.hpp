...
public:
  ...
  uint32_t size() const { return _num_constants + _num_leaves + static_cast<uint32_t>( _inner.size() ); }
  uint32_t num_pis() const { return _num_leaves; }
  uint32_t num_pos() const { return _empty ? 0u : 1u; }
  uint32_t num_gates() const { return static_cast<uint32_t>( _inner.size() ); }
  ...
  std::vector<node> const& leaves() const { return _leaves; }
  std::vector<node> const& gates() const { return _inner; }
  ...
  void update_mffc() { recompute(); }

private:
  static bool is_constant( node n ) { return n == 0u; }
